...
Plugin URI: http://dennishoppe.de/en/wordpress-plugins/auto-image-resizer
Version: 1.2.2.0
Author: Dennis Hoppe
...
class wp_plugin_auto_image_resizer {
  ...
  function Add_Options_Page (){
    ...
    // Add option boxes
    $this->Add_Option_Box ( $this->t('Image Size'), DirName(__FILE__).'/options-page/option-box-image-size.php' );
    $this->Add_Option_Box ( $this->t('Image quality'), DirName(__FILE__).'/options-page/option-box-image-quality.php' );
    $this->Add_Option_Box ( __('Update'), DirName(__FILE__).'/options-page/option-box-update.php', 'side' );
  ...
  function Get_Max_Dimensions(){
		...
		ForEach (Get_Intermediate_Image_Sizes() as $image_size){
			...
			Else {
				$arr_width[] = IntVal(Get_Option($image_size.'_size_w'));
				$arr_height[] = IntVal(Get_Option($image_size.'_size_h'));
			}
		}
  ...
  function Catch_Upload($attachment_id){
    ...
    <script type="text/javascript">jQuery(document).ready(function($){
      ...
      uploader.settings.resize = <?php Echo JSON_Encode($resize_options) ?>;
		...
		<p>
      <input type="checkbox" checked="checked" disabled="disabled" id="sepia">
